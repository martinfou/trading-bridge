#!/bin/bash
# Run GoldTurtleSizeOverlay (36e résultat, 15 sept 2026) — sélecteur (gate) vs amplificateur (taille).
...
# ⚠️ stderr -> /dev/null : les logs ERROR (persistence SQLite absente du CP) s'intercalent
# AU MILIEU des lignes de tableau et corrompent la sortie si on les mélange (2>&1).
timeout --preserve-status 1200 java -cp "$CP" com.martinfou.trading.examples.RunGoldTurtleSizeOverlay $MODE 2>/dev/null
