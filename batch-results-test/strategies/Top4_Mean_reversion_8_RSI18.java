...
/**
 * Top4_Mean_reversion_8_RSI18 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(18, CLOSE)</li>
 ...
 * </ul>
 */
public class Top4_Mean_reversion_8_RSI18 implements Strategy {
    ...
    private static final int TAKE_PROFIT_POINTS = 182;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top4_Mean_reversion_8_RSI18 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top4_Mean_reversion_8_RSI18(String symbol) {
        this.name = "Top4_Mean_reversion_8_RSI18";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 72) return;

        // --- Compute indicator values ---
        double rsi_18_close = rsi(history, 18);
        double sma_72_close = sma(history, 72, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_18_close = rsiPrev(history, 18);
        double prevSma_72_close = smaPrev(history, 72, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_18_close <= prevSma_72_close && rsi_18_close > sma_72_close) {
                entryTriggered = true;
            }
