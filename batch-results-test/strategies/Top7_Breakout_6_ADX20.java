...
/**
 * Top7_Breakout_6_ADX20 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>ADX(20, CLOSE)</li>
 ...
 * </ul>
 */
public class Top7_Breakout_6_ADX20 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 58;
    private static final int TAKE_PROFIT_POINTS = 130;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top7_Breakout_6_ADX20 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top7_Breakout_6_ADX20(String symbol) {
        this.name = "Top7_Breakout_6_ADX20";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        double adx_20_close = adx(history, 20);
        double ema_7_close = ema(history, 7, "CLOSE");
        double atr_17_close = atr(history, 17);

        // --- Previous indicator values ---
        double prevAdx_20_close = adxPrev(history, 20);
        double prevEma_7_close = emaPrev(history, 7, "CLOSE");
        double prevAtr_17_close = atrPrev(history, 17);

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevAdx_20_close <= prevEma_7_close && adx_20_close > ema_7_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (atr_17_close < prevAtr_17_close) {
                exitTriggered = true;
            }
