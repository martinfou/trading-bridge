...
/**
 * Top10_Trend_following_11_SMA63 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(63, CLOSE)</li>
 ...
 * </ul>
 */
public class Top10_Trend_following_11_SMA63 implements Strategy {
    ...
    private static final int TAKE_PROFIT_POINTS = 425;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top10_Trend_following_11_SMA63 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top10_Trend_following_11_SMA63(String symbol) {
        this.name = "Top10_Trend_following_11_SMA63";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 93) return;

        // --- Compute indicator values ---
        double sma_63_close = sma(history, 63, "CLOSE");
        double sma_93_close = sma(history, 93, "CLOSE");
        double sma_73_close = sma(history, 73, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_63_close = smaPrev(history, 63, "CLOSE");
        double prevSma_93_close = smaPrev(history, 93, "CLOSE");
        double prevSma_73_close = smaPrev(history, 73, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_63_close <= prevSma_93_close && sma_63_close > sma_93_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (prevSma_73_close >= prevSma_93_close && sma_73_close < sma_93_close) {
                exitTriggered = true;
            }
