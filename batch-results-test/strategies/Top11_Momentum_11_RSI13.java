...
/**
 * Top11_Momentum_11_RSI13 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(13, CLOSE)</li>
 ...
 * </ul>
 */
public class Top11_Momentum_11_RSI13 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 65;
    private static final int TAKE_PROFIT_POINTS = 333;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top11_Momentum_11_RSI13 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top11_Momentum_11_RSI13(String symbol) {
        this.name = "Top11_Momentum_11_RSI13";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 13) return;

        // --- Compute indicator values ---
        double rsi_13_close = rsi(history, 13);
        double adx_11_close = adx(history, 11);
        double ema_8_close = ema(history, 8, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_13_close = rsiPrev(history, 13);
        double prevAdx_11_close = adxPrev(history, 11);
        double prevEma_8_close = emaPrev(history, 8, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_13_close <= prevAdx_11_close && rsi_13_close > adx_11_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (ema_8_close < prevEma_8_close) {
                exitTriggered = true;
            }
