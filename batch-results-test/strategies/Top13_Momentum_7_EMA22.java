...
/**
 * Top13_Momentum_7_EMA22 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(22, CLOSE)</li>
 ...
 * </ul>
 */
public class Top13_Momentum_7_EMA22 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 139;
    private static final int TAKE_PROFIT_POINTS = 408;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top13_Momentum_7_EMA22 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top13_Momentum_7_EMA22(String symbol) {
        this.name = "Top13_Momentum_7_EMA22";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 43) return;

        // --- Compute indicator values ---
        double ema_22_close = ema(history, 22, "CLOSE");
        double ema_43_close = ema(history, 43, "CLOSE");
        double rsi_13_close = rsi(history, 13);

        // --- Previous indicator values ---
        double prevEma_22_close = emaPrev(history, 22, "CLOSE");
        double prevEma_43_close = emaPrev(history, 43, "CLOSE");
        double prevRsi_13_close = rsiPrev(history, 13);

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_22_close <= prevEma_43_close && ema_22_close > ema_43_close) {
                entryTriggered = true;
            }
