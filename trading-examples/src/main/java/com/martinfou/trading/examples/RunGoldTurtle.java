...
/**
 * RunGoldTurtle — Backtest GoldTurtleTrendStrategy (Turtle Donchian 55/20 sur
 * XAU/USD) AVEC coûts (commission $0.07 + slippage 0.01%) — jamais sans coûts.
 *
 ...
 *
 * Usage:
 *   java -cp "$CP" com.martinfou.trading.examples.RunGoldTurtle
 *   java -cp "$CP" com.martinfou.trading.examples.RunGoldTurtle --sweep
 */
public class RunGoldTurtle {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("==================================================");
        System.out.println("GOLD TURTLE TREND — Donchian 55/20, XAU/USD H1");
        ...
        for (String symbol : PAIRS) {
            ...
            var strategy = new GoldTurtleTrendStrategy("GoldTurtleTrend", symbol);
            BacktestResult r = RunContext.forStrategy(null, "GoldTurtleTrend", strategy, symbol,
    ...
    private static void runRegime(BacktestExecutionCost cost) throws Exception {
        ...
        System.out.println("=== RÉGIME DE MARCHÉ XAU_USD (bull/bear/bull2) ===");
    ...
    private static void runSweep(BacktestExecutionCost cost) throws Exception {
        ...
        System.out.println("=== SWEEP PARAMÉTRIQUE XAU_USD (entrée × sortie) ===");
        System.out.printf("%-8s %-8s %-6s %-6s %-6s %-7s %-12s%n",
            "ENTRY", "EXIT", "PF", "WR%", "DD%", "TRADES", "NET$");
        for (int e : entries) {
            for (int x : exits) {
                var strategy = new GoldTurtleTrendStrategy("GoldTurtleTrend", symbol, e, x);
                BacktestResult r = RunContext.forStrategy(null, "GoldTurtleTrend", strategy, symbol,
                    RunMode.BACKTEST, bars, CAPITAL, null, cost).run();
