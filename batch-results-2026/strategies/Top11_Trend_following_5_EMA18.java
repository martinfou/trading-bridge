...
/**
 * Top11_Trend_following_5_EMA18 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(18, CLOSE)</li>
 ...
 * </ul>
 */
public class Top11_Trend_following_5_EMA18 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 127;
    private static final int TAKE_PROFIT_POINTS = 376;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top11_Trend_following_5_EMA18 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top11_Trend_following_5_EMA18(String symbol) {
        this.name = "Top11_Trend_following_5_EMA18";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 23) return;

        // --- Compute indicator values ---
        double ema_18_close = ema(history, 18, "CLOSE");
        double ema_23_close = ema(history, 23, "CLOSE");
        double sma_15_close = sma(history, 15, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_18_close = emaPrev(history, 18, "CLOSE");
        double prevEma_23_close = emaPrev(history, 23, "CLOSE");
        double prevSma_15_close = smaPrev(history, 15, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_18_close <= prevEma_23_close && ema_18_close > ema_23_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_15_close < prevSma_15_close) {
                exitTriggered = true;
            }
