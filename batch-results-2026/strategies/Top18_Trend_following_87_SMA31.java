...
/**
 * Top18_Trend_following_87_SMA31 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(31, CLOSE)</li>
 ...
 * </ul>
 */
public class Top18_Trend_following_87_SMA31 implements Strategy {
    ...
    /**
     * Creates a new Top18_Trend_following_87_SMA31 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top18_Trend_following_87_SMA31(String symbol) {
        this.name = "Top18_Trend_following_87_SMA31";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 61) return;

        // --- Compute indicator values ---
        double sma_31_close = sma(history, 31, "CLOSE");
        double sma_61_close = sma(history, 61, "CLOSE");
        double sma_41_close = sma(history, 41, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_31_close = smaPrev(history, 31, "CLOSE");
        double prevSma_61_close = smaPrev(history, 61, "CLOSE");
        double prevSma_41_close = smaPrev(history, 41, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_31_close <= prevSma_61_close && sma_31_close > sma_61_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (prevSma_41_close >= prevSma_61_close && sma_41_close < sma_61_close) {
                exitTriggered = true;
            }
