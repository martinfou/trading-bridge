...
/**
 * Top7_Momentum_101_RSI15 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(15, CLOSE)</li>
 ...
 * </ul>
 */
public class Top7_Momentum_101_RSI15 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 113;
    private static final int TAKE_PROFIT_POINTS = 352;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top7_Momentum_101_RSI15 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top7_Momentum_101_RSI15(String symbol) {
        this.name = "Top7_Momentum_101_RSI15";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        double rsi_15_close = rsi(history, 15);
        double adx_11_close = adx(history, 11);
        double ema_5_close = ema(history, 5, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_15_close = rsiPrev(history, 15);
        double prevAdx_11_close = adxPrev(history, 11);
        double prevEma_5_close = emaPrev(history, 5, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_15_close <= prevAdx_11_close && rsi_15_close > adx_11_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (ema_5_close < prevEma_5_close) {
                exitTriggered = true;
            }
