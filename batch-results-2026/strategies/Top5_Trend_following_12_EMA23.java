...
/**
 * Top5_Trend_following_12_EMA23 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(23, CLOSE)</li>
 ...
 * </ul>
 */
public class Top5_Trend_following_12_EMA23 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 95;
    private static final int TAKE_PROFIT_POINTS = 191;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top5_Trend_following_12_EMA23 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top5_Trend_following_12_EMA23(String symbol) {
        this.name = "Top5_Trend_following_12_EMA23";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 44) return;

        // --- Compute indicator values ---
        double ema_23_close = ema(history, 23, "CLOSE");
        double ema_44_close = ema(history, 44, "CLOSE");
        double sma_16_close = sma(history, 16, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_23_close = emaPrev(history, 23, "CLOSE");
        double prevEma_44_close = emaPrev(history, 44, "CLOSE");
        double prevSma_16_close = smaPrev(history, 16, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_23_close <= prevEma_44_close && ema_23_close > ema_44_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_16_close < prevSma_16_close) {
                exitTriggered = true;
            }
