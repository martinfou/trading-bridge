...
/**
 * Top9_Trend_following_26_EMA29 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(29, CLOSE)</li>
 ...
 * </ul>
 */
public class Top9_Trend_following_26_EMA29 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 166;
    private static final int TAKE_PROFIT_POINTS = 456;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top9_Trend_following_26_EMA29 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top9_Trend_following_26_EMA29(String symbol) {
        this.name = "Top9_Trend_following_26_EMA29";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 51) return;

        // --- Compute indicator values ---
        double ema_29_close = ema(history, 29, "CLOSE");
        double ema_51_close = ema(history, 51, "CLOSE");
        double sma_28_close = sma(history, 28, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_29_close = emaPrev(history, 29, "CLOSE");
        double prevEma_51_close = emaPrev(history, 51, "CLOSE");
        double prevSma_28_close = smaPrev(history, 28, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_29_close <= prevEma_51_close && ema_29_close > ema_51_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_28_close < prevSma_28_close) {
                exitTriggered = true;
            }
