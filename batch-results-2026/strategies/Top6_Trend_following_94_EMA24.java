...
/**
 * Top6_Trend_following_94_EMA24 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(24, CLOSE)</li>
 ...
 * </ul>
 */
public class Top6_Trend_following_94_EMA24 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 104;
    private static final int TAKE_PROFIT_POINTS = 261;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top6_Trend_following_94_EMA24 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top6_Trend_following_94_EMA24(String symbol) {
        this.name = "Top6_Trend_following_94_EMA24";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 31) return;

        // --- Compute indicator values ---
        double ema_24_close = ema(history, 24, "CLOSE");
        double ema_31_close = ema(history, 31, "CLOSE");
        double sma_19_close = sma(history, 19, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_24_close = emaPrev(history, 24, "CLOSE");
        double prevEma_31_close = emaPrev(history, 31, "CLOSE");
        double prevSma_19_close = smaPrev(history, 19, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_24_close <= prevEma_31_close && ema_24_close > ema_31_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_19_close < prevSma_19_close) {
                exitTriggered = true;
            }
