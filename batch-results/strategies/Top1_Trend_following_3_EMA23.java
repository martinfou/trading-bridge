...
/**
 * Top1_Trend_following_3_EMA23 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(23, CLOSE)</li>
 ...
 * </ul>
 */
public class Top1_Trend_following_3_EMA23 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 153;
    private static final int TAKE_PROFIT_POINTS = 227;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top1_Trend_following_3_EMA23 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top1_Trend_following_3_EMA23(String symbol) {
        this.name = "Top1_Trend_following_3_EMA23";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 28) return;

        // --- Compute indicator values ---
        double ema_23_close = ema(history, 23, "CLOSE");
        double adx_9_close = adx(history, 9);
        double ema_28_close = ema(history, 28, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_23_close = emaPrev(history, 23, "CLOSE");
        double prevAdx_9_close = adxPrev(history, 9);
        double prevEma_28_close = emaPrev(history, 28, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_23_close <= prevAdx_9_close && ema_23_close > adx_9_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (ema_28_close < prevEma_28_close) {
                exitTriggered = true;
            }
