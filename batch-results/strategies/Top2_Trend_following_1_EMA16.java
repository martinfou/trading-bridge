...
/**
 * Top2_Trend_following_1_EMA16 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(16, CLOSE)</li>
 ...
 * </ul>
 */
public class Top2_Trend_following_1_EMA16 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 183;
    private static final int TAKE_PROFIT_POINTS = 314;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top2_Trend_following_1_EMA16 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top2_Trend_following_1_EMA16(String symbol) {
        this.name = "Top2_Trend_following_1_EMA16";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 82) return;

        // --- Compute indicator values ---
        double ema_16_close = ema(history, 16, "CLOSE");
        double ema_45_close = ema(history, 45, "CLOSE");
        double sma_82_close = sma(history, 82, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_16_close = emaPrev(history, 16, "CLOSE");
        double prevEma_45_close = emaPrev(history, 45, "CLOSE");
        double prevSma_82_close = smaPrev(history, 82, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_16_close <= prevEma_45_close && ema_16_close > ema_45_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_82_close < prevSma_82_close) {
                exitTriggered = true;
            }
