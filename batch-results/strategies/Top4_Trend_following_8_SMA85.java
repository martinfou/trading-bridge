...
/**
 * Top4_Trend_following_8_SMA85 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(85, CLOSE)</li>
 ...
 * </ul>
 */
public class Top4_Trend_following_8_SMA85 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 133;
    private static final int TAKE_PROFIT_POINTS = 425;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top4_Trend_following_8_SMA85 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top4_Trend_following_8_SMA85(String symbol) {
        this.name = "Top4_Trend_following_8_SMA85";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 85) return;

        // --- Compute indicator values ---
        double sma_85_close = sma(history, 85, "CLOSE");
        double adx_7_close = adx(history, 7);
        double sma_42_close = sma(history, 42, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_85_close = smaPrev(history, 85, "CLOSE");
        double prevAdx_7_close = adxPrev(history, 7);
        double prevSma_42_close = smaPrev(history, 42, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_85_close <= prevAdx_7_close && sma_85_close > adx_7_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_42_close < prevSma_42_close) {
                exitTriggered = true;
            }
