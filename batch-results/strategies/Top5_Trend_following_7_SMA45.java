...
/**
 * Top5_Trend_following_7_SMA45 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(45, CLOSE)</li>
 ...
 * </ul>
 */
public class Top5_Trend_following_7_SMA45 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 194;
    private static final int TAKE_PROFIT_POINTS = 262;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top5_Trend_following_7_SMA45 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top5_Trend_following_7_SMA45(String symbol) {
        this.name = "Top5_Trend_following_7_SMA45";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        double sma_45_close = sma(history, 45, "CLOSE");
        double adx_19_close = adx(history, 19);
        double sma_22_close = sma(history, 22, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_45_close = smaPrev(history, 45, "CLOSE");
        double prevAdx_19_close = adxPrev(history, 19);
        double prevSma_22_close = smaPrev(history, 22, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_45_close <= prevAdx_19_close && sma_45_close > adx_19_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_22_close < prevSma_22_close) {
                exitTriggered = true;
            }
