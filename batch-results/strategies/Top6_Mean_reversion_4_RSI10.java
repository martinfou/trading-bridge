...
/**
 * Top6_Mean_reversion_4_RSI10 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(10, CLOSE)</li>
 ...
 * </ul>
 */
public class Top6_Mean_reversion_4_RSI10 implements Strategy {
    ...
    private static final int TAKE_PROFIT_POINTS = 146;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top6_Mean_reversion_4_RSI10 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top6_Mean_reversion_4_RSI10(String symbol) {
        this.name = "Top6_Mean_reversion_4_RSI10";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        double rsi_10_close = rsi(history, 10);
        double ema_16_close = ema(history, 16, "CLOSE");
        double sma_13_close = sma(history, 13, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_10_close = rsiPrev(history, 10);
        double prevEma_16_close = emaPrev(history, 16, "CLOSE");
        double prevSma_13_close = smaPrev(history, 13, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_10_close <= prevEma_16_close && rsi_10_close > ema_16_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_13_close < prevSma_13_close) {
                exitTriggered = true;
            }
