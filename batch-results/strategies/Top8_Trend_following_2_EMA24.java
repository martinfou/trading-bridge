...
/**
 * Top8_Trend_following_2_EMA24 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(24, CLOSE)</li>
 ...
 * </ul>
 */
public class Top8_Trend_following_2_EMA24 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 124;
    private static final int TAKE_PROFIT_POINTS = 208;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top8_Trend_following_2_EMA24 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top8_Trend_following_2_EMA24(String symbol) {
        this.name = "Top8_Trend_following_2_EMA24";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 38) return;

        // --- Compute indicator values ---
        double ema_24_close = ema(history, 24, "CLOSE");
        double ema_38_close = ema(history, 38, "CLOSE");
        double sma_21_close = sma(history, 21, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_24_close = emaPrev(history, 24, "CLOSE");
        double prevEma_38_close = emaPrev(history, 38, "CLOSE");
        double prevSma_21_close = smaPrev(history, 21, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_24_close <= prevEma_38_close && ema_24_close > ema_38_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_21_close < prevSma_21_close) {
                exitTriggered = true;
            }
