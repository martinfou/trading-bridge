...
/**
 * Top9_Mean_reversion_4_RSI16 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(16, CLOSE)</li>
 ...
 * </ul>
 */
public class Top9_Mean_reversion_4_RSI16 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 72;
    private static final int TAKE_PROFIT_POINTS = 200;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top9_Mean_reversion_4_RSI16 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top9_Mean_reversion_4_RSI16(String symbol) {
        this.name = "Top9_Mean_reversion_4_RSI16";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 49) return;

        // --- Compute indicator values ---
        double rsi_16_close = rsi(history, 16);
        double sma_49_close = sma(history, 49, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_16_close = rsiPrev(history, 16);
        double prevSma_49_close = smaPrev(history, 49, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_16_close <= prevSma_49_close && rsi_16_close > sma_49_close) {
                entryTriggered = true;
            }
