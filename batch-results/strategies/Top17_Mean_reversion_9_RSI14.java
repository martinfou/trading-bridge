...
/**
 * Top17_Mean_reversion_9_RSI14 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(14, CLOSE)</li>
 ...
 * </ul>
 */
public class Top17_Mean_reversion_9_RSI14 implements Strategy {
    ...
    /**
     * Creates a new Top17_Mean_reversion_9_RSI14 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top17_Mean_reversion_9_RSI14(String symbol) {
        this.name = "Top17_Mean_reversion_9_RSI14";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 86) return;

        // --- Compute indicator values ---
        double rsi_14_close = rsi(history, 14);
        double ema_16_close = ema(history, 16, "CLOSE");
        double sma_86_close = sma(history, 86, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_14_close = rsiPrev(history, 14);
        double prevEma_16_close = emaPrev(history, 16, "CLOSE");
        double prevSma_86_close = smaPrev(history, 86, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_14_close <= prevEma_16_close && rsi_14_close > ema_16_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_86_close < prevSma_86_close) {
                exitTriggered = true;
            }
