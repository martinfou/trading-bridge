...
/**
 * Top10_Mean_reversion_5_RSI13 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(13, CLOSE)</li>
 ...
 * </ul>
 */
public class Top10_Mean_reversion_5_RSI13 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 41;
    private static final int TAKE_PROFIT_POINTS = 150;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top10_Mean_reversion_5_RSI13 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top10_Mean_reversion_5_RSI13(String symbol) {
        this.name = "Top10_Mean_reversion_5_RSI13";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 34) return;

        // --- Compute indicator values ---
        double rsi_13_close = rsi(history, 13);
        double ema_12_close = ema(history, 12, "CLOSE");
        double sma_34_close = sma(history, 34, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_13_close = rsiPrev(history, 13);
        double prevEma_12_close = emaPrev(history, 12, "CLOSE");
        double prevSma_34_close = smaPrev(history, 34, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_13_close <= prevEma_12_close && rsi_13_close > ema_12_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_34_close < prevSma_34_close) {
                exitTriggered = true;
            }
