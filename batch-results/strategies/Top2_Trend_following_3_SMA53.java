...
/**
 * Top2_Trend_following_3_SMA53 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(53, CLOSE)</li>
 ...
 * </ul>
 */
public class Top2_Trend_following_3_SMA53 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 120;
    private static final int TAKE_PROFIT_POINTS = 278;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top2_Trend_following_3_SMA53 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top2_Trend_following_3_SMA53(String symbol) {
        this.name = "Top2_Trend_following_3_SMA53";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 53) return;

        // --- Compute indicator values ---
        double sma_53_close = sma(history, 53, "CLOSE");
        double adx_7_close = adx(history, 7);
        double sma_26_close = sma(history, 26, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_53_close = smaPrev(history, 53, "CLOSE");
        double prevAdx_7_close = adxPrev(history, 7);
        double prevSma_26_close = smaPrev(history, 26, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_53_close <= prevAdx_7_close && sma_53_close > adx_7_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_26_close < prevSma_26_close) {
                exitTriggered = true;
            }
