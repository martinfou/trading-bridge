...
/**
 * Top9_Trend_following_3_SMA54 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>SMA(54, CLOSE)</li>
 ...
 * </ul>
 */
public class Top9_Trend_following_3_SMA54 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 33;
    private static final int TAKE_PROFIT_POINTS = 154;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top9_Trend_following_3_SMA54 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top9_Trend_following_3_SMA54(String symbol) {
        this.name = "Top9_Trend_following_3_SMA54";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 84) return;

        // --- Compute indicator values ---
        double sma_54_close = sma(history, 54, "CLOSE");
        double sma_84_close = sma(history, 84, "CLOSE");
        double sma_64_close = sma(history, 64, "CLOSE");

        // --- Previous indicator values ---
        double prevSma_54_close = smaPrev(history, 54, "CLOSE");
        double prevSma_84_close = smaPrev(history, 84, "CLOSE");
        double prevSma_64_close = smaPrev(history, 64, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevSma_54_close <= prevSma_84_close && sma_54_close > sma_84_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (prevSma_64_close >= prevSma_84_close && sma_64_close < sma_84_close) {
                exitTriggered = true;
            }
