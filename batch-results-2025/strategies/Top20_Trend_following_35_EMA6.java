...
/**
 * Top20_Trend_following_35_EMA6 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(6, CLOSE)</li>
 ...
 * </ul>
 */
public class Top20_Trend_following_35_EMA6 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 142;
    private static final int TAKE_PROFIT_POINTS = 197;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top20_Trend_following_35_EMA6 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top20_Trend_following_35_EMA6(String symbol) {
        this.name = "Top20_Trend_following_35_EMA6";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        double ema_6_close = ema(history, 6, "CLOSE");
        double ema_12_close = ema(history, 12, "CLOSE");
        double sma_24_close = sma(history, 24, "CLOSE");

        // --- Previous indicator values ---
        double prevEma_6_close = emaPrev(history, 6, "CLOSE");
        double prevEma_12_close = emaPrev(history, 12, "CLOSE");
        double prevSma_24_close = smaPrev(history, 24, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_6_close <= prevEma_12_close && ema_6_close > ema_12_close) {
                entryTriggered = true;
            }
        ...
        if (hasPosition) {
            ...
            if (sma_24_close < prevSma_24_close) {
                exitTriggered = true;
            }
