...
/**
 * Top15_Momentum_34_EMA24 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>EMA(24, CLOSE)</li>
 ...
 * </ul>
 */
public class Top15_Momentum_34_EMA24 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 192;
    private static final int TAKE_PROFIT_POINTS = 384;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top15_Momentum_34_EMA24 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top15_Momentum_34_EMA24(String symbol) {
        this.name = "Top15_Momentum_34_EMA24";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 36) return;

        // --- Compute indicator values ---
        double ema_24_close = ema(history, 24, "CLOSE");
        double ema_36_close = ema(history, 36, "CLOSE");
        double rsi_19_close = rsi(history, 19);

        // --- Previous indicator values ---
        double prevEma_24_close = emaPrev(history, 24, "CLOSE");
        double prevEma_36_close = emaPrev(history, 36, "CLOSE");
        double prevRsi_19_close = rsiPrev(history, 19);

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevEma_24_close <= prevEma_36_close && ema_24_close > ema_36_close) {
                entryTriggered = true;
            }
