...
/**
 * Top10_Mean_reversion_32_RSI20 — auto-generated from a genetic-algorithm chromosome.
 *
 * <p>Entry conditions:
 * <ul>
 *   <li>RSI(20, CLOSE)</li>
 ...
 * </ul>
 */
public class Top10_Mean_reversion_32_RSI20 implements Strategy {
    ...
    private static final int STOP_LOSS_POINTS = 46;
    private static final int TAKE_PROFIT_POINTS = 166;
    private static final double PRICE_SCALE = 0.0001;

    /**
     * Creates a new Top10_Mean_reversion_32_RSI20 strategy for the given symbol.
     *
     * @param symbol the trading symbol (e.g. "EUR_USD")
     */
    public Top10_Mean_reversion_32_RSI20(String symbol) {
        this.name = "Top10_Mean_reversion_32_RSI20";
        this.symbol = symbol;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (history.size() < 63) return;

        // --- Compute indicator values ---
        double rsi_20_close = rsi(history, 20);
        double sma_63_close = sma(history, 63, "CLOSE");

        // --- Previous indicator values ---
        double prevRsi_20_close = rsiPrev(history, 20);
        double prevSma_63_close = smaPrev(history, 63, "CLOSE");

        // --- Entry conditions ---
        if (!hasPosition) {
            boolean entryTriggered = false;
            if (prevRsi_20_close <= prevSma_63_close && rsi_20_close > sma_63_close) {
                entryTriggered = true;
            }
