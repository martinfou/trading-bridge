...
/**
 * AugustRiskCheck — Vérifie la robustesse du pattern "August curse" (août baissier)
 * sur les paires risk (AUD_USD, NZD_USD, GBP_JPY, GBP_USD, EUR_USD).
 *
 * Méthode (issue du XauAugustCheck, 2026-08-05) :
 * 1. Rendement d'août année par année (2006-2026) + split chronologique IS/OOS
 * 2. Contrôle "même hit rate sur mois voisins" (Jul/Aug/Sep) — signature d'une
 *    tendance longue vs un vrai edge calendaire
 * 3. Seul un hit rate août DISTINCT des mois adjacents + stable OOS est exploitable.
 */
public class AugustRiskCheck {
    private static final String BARS_DIR = "data/historical/bars";

    public static void main(String[] args) throws Exception {
        String[] symbols = {"AUD_USD", "NZD_USD", "GBP_JPY", "GBP_USD", "EUR_USD"};
        int fromYear = 2006, toYear = 2026;
        var barsDir = Paths.get(BARS_DIR);

        for (String symbol : symbols) {
            System.out.println("\n========== " + symbol + " — August curse check ==========");
            ...
            List<Double> allAug = new ArrayList<>(), isAug = new ArrayList<>(), oosAug = new ArrayList<>();
            ...
            // Contrôle : mois voisins + tous les mois
            System.out.println("--- Contrôle mois voisins (Jul / Aug / Sep) ---");
