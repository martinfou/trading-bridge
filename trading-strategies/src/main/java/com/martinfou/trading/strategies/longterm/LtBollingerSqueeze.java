 ...
 *   - Fixed fractional units (1000 base)
...
public class LtBollingerSqueeze implements Strategy {
    ...
    private static final long BASE_UNITS = 1000;
    ...
    public void onBar(Bar bar) {
        ...
        if (close > sma) {
            // Bullish breakout from squeeze — BUY
            direction = Order.Side.BUY;
            entrySl = close - atr * ATR_SL_MULT;
            entryTp = close + atr * ATR_TP_MULT;
            pending.add(new Order(symbol, Order.Side.BUY, Order.Type.MARKET, BASE_UNITS, close)
                .withStopLoss(entrySl).withTakeProfit(entryTp));
            inTrade = true;
            lastTradeDate = barDate;
        } else if (close < sma) {
            // Bearish breakout from squeeze — SELL
            direction = Order.Side.SELL;
            entrySl = close + atr * ATR_SL_MULT;
            entryTp = close - atr * ATR_TP_MULT;
            pending.add(new Order(symbol, Order.Side.SELL, Order.Type.MARKET, BASE_UNITS, close)
                .withStopLoss(entrySl).withTakeProfit(entryTp));
            inTrade = true;
            lastTradeDate = barDate;
        }
    ...
    private void exitTrade(double price) {
        ...
        pending.add(new Order(symbol, exit, Order.Type.MARKET, BASE_UNITS, price).closeOnly());
        inTrade = false;
    }
