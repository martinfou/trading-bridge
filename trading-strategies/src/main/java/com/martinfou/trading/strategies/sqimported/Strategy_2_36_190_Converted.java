 ...
 * SL: 145 pips, PT: 395 pips
 * Expiration: 198 bars
 */
public class Strategy_2_36_190_Converted implements Strategy {
    ...
    @Override
    public String name() {
        return "Strategy_2_36_190_Converted";
    }
    ...
    private final String name = "2.36.190";
    ...
    public void onBar(Bar bar) {
        ...
        // LongEntrySignal:
                ...
                if (longEntry && !(activeOrder != null) && !(false)) {
                    // Entry: High(2) + 1.8 * BBRange(10, 0.9, CLOSE, 1)
                    double high2 = getBar(2).high();
                    double bbRange = calcBBRange(history, 10, 0.9, "close", 1);
        
                    if (Double.isNaN(bbRange)) return;
        
                    double entryPrice = high2 + (1.8 * bbRange);
                    {
                    double ep = entryPrice;
                    double sl = ep - (145 * PIP);
                    double tp = ep + (395 * PIP);
