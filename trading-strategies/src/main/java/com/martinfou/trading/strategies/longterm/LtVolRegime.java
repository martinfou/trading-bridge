...
public class LtVolRegime implements Strategy {
    ...
    public LtVolRegime(String name, String symbol) {
        this.name = name;
        this.symbol = symbol;
        this.positionSize = POSITION_SIZE;
    }
    ...
    public void onBar(Bar bar) {
        ...
        if (atrRatio > HIGH_VOL_THRESHOLD) {
            ...
            if (price > ema50) {
                ...
                pending.add(new Order(symbol, Order.Side.BUY, Order.Type.MARKET, positionSize, price));
                inTrade = true;
                ...
                stopLoss = price + atrShort * SL_MULT;
                takeProfit = price - atrShort * TP_MULT;
                pending.add(new Order(symbol, Order.Side.SELL, Order.Type.MARKET, positionSize, price));
                inTrade = true;
