...
public class TurnOfMonthFlowStrategy implements Strategy {
    ...
    // Trading-day counting MUST use UTC (Dukascopy H1 bars are UTC-timestamped).
    // America/New_York shifts bars 00:00-04:59 UTC into the previous day/month,
    // desynchronizing the month-end window detection. See
    // references/calendar-strategy-timezone-pitfalls.md.
    ...
    public void onBar(Bar bar) {
        ...
        // Track month for trade counting — MUST use UTC for calendar day counting
        // (America/New_York shifts 00:00-04:59 UTC bars into the previous month)
        int barMonth = bar.timestamp().atZone(utc).getMonthValue();
        int barYear = bar.timestamp().atZone(utc).getYear();

        // Reset the per-month trade counter when the month changes.
        // Without this, tradeCountThisMonth stays >= 1 forever after the first
        // trade, and the strategy never enters again (1 trade in 20 years bug).
     ...
     * ⚠️ Uses UTC for calendar day counting — America/New_York would shift
     * 00:00-04:59 UTC bars into the previous month (timezone pitfall).
     */
